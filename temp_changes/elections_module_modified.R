...
#' Get custom canton colors
#' @return A named vector of colors for each canton
...
elections_server <- function(id, selected_data) {
  moduleServer(id, function(input, output, session) {
    ...
    observe({
      ...
      # Get full canton names in German and sort them alphabetically
      canton_names_de <- sapply(canton_numbers, function(id) {
        get_canton_name(get_canton_abbr(id), "de")
      })
      
      # Sort the canton numbers based on alphabetically sorted German names
      sorted_indices <- order(canton_names_de)
      sorted_canton_numbers <- canton_numbers[sorted_indices]
      sorted_canton_names <- canton_names_de[sorted_indices]
      
      # Create named choices with the sorted full names
      choices <- setNames(sorted_canton_numbers, sorted_canton_names)
    ...
    update_filtered_data <- function() {
      ...
      if (input$canton_mode == "select") {
        ...
        if (is.null(input$canton_select) || length(input$canton_select) == 0) {
          ...
          # With our updated code, input$canton_select now contains canton numbers as strings
    ...
    output$time_trend_plot <- renderPlotly({
      ...
      # Check if we are showing specific cantons
      ...
      if (input$variables %in% c("rae", "partfrakt", "parl_party", "max_sitzzahl_parl", "reg_konk", "konk_2", 
        ...
        # For "all cantons" view or non-parliamentary election variables, show the average across all cantons
      ...
      if (input$variables == "turnout_e") {
        # Check if we're showing specific cantons for parliamentary elections
        if (!is.null(selected_cantons) && input$election_type == "parliament") {
          # Show individual canton values when specific cantons are selected
          canton_data <- data %>%
            # Convert "." to NA and then to numeric
            mutate(turnout_e = ifelse(turnout_e == ".", NA, turnout_e),
                   turnout_e = as.numeric(turnout_e)) %>%
            filter(!is.na(turnout_e), kantonnr %in% selected_cantons) %>%
            # Add canton information
            mutate(
              canton_abbrev = sapply(kantonnr, get_canton_abbr),
              canton_name = sapply(canton_abbrev, function(abbr) get_canton_name(abbr, "de"))
          ...
          p <- plot_ly() %>%
            layout(
              title = "Kantonsspezifische Wahlbeteiligung bei Parlamentswahlen über die Zeit",
              xaxis = list(
                title = "Jahr",
                tickmode = "linear",
                dtick = 2
          ...
          for (canton_nr in selected_cantons) {
            ...
            if (nrow(canton_data_filtered) > 0) {
              ...
              p <- p %>% add_trace(
                ...
                hoverinfo = "text",
                text = ~paste(canton_name, "<br>Jahr:", get(year_col), 
                              "<br>Wahlbeteiligung:", round(turnout_e, 1), "%")
              )
            }
          }
        ...
        # For "all cantons" view, calculate average turnout per year
        turnout_data <- data %>%
          # Convert "." to NA and then to numeric
          mutate(turnout_e = ifelse(turnout_e == ".", NA, turnout_e),
                 turnout_e = as.numeric(turnout_e)) %>%
      ...
      if (input$variables == "parlegisl") {
        ...
        if (!is.null(selected_cantons) && input$election_type == "parliament") {
          ...
          canton_data <- data %>%
            ...
            mutate(parlegisl = ifelse(parlegisl == ".", NA, parlegisl),
                   parlegisl = as.numeric(parlegisl)) %>%
            filter(!is.na(parlegisl), kantonnr %in% selected_cantons) %>%
            # Add canton information
            mutate(
              canton_abbrev = sapply(kantonnr, get_canton_abbr),
              canton_name = sapply(canton_abbrev, function(abbr) get_canton_name(abbr, "de"))
          ...
          p <- plot_ly() %>%
            layout(
              title = "Kantonsspezifische Amtsdauer des Parlaments über die Zeit",
              xaxis = list(
                title = "Jahr",
                tickmode = "linear",
                dtick = 2
        ...
        # For "all cantons" view, calculate average term length per year
        term_data <- data %>%
          # Convert "." to NA and then to numeric
          mutate(parlegisl = ifelse(parlegisl == ".", NA, parlegisl),
                 parlegisl = as.numeric(parlegisl)) %>%
      ...
      if (input$variables == "parl_election") {
        # Check if we're showing specific cantons for parliamentary elections
        if (!is.null(selected_cantons) && input$election_type == "parliament") {
          # For parliamentary elections with specific cantons, show individual canton values
          canton_data <- data %>%
            ...
            filter(!is.na(parl_election), kantonnr %in% selected_cantons) %>%
            # Add canton information
            mutate(
              canton_abbrev = sapply(kantonnr, get_canton_abbr),
              canton_name = sapply(canton_abbrev, function(abbr) get_canton_name(abbr, "de"))
          ...
          canton_colors <- c(
            "AG" = "#006AB2", "AI" = "#006AB2", "AR" = "#006AB2", "BE" = "#E30613", 
            "BL" = "#006AB2", "BS" = "#006AB2", "FR" = "#006AB2", "GE" = "#006AB2", 
            "GL" = "#006AB2", "GR" = "#006AB2", "JU" = "#006AB2", "LU" = "#006AB2", 
            "NE" = "#006AB2", "NW" = "#006AB2", "OW" = "#006AB2", "SG" = "#006AB2", 
            "SH" = "#006AB2", "SO" = "#E30613", "SZ" = "#E30613", "TG" = "#006AB2", 
            "TI" = "#E30613", "UR" = "#006AB2", "VD" = "#006AB2", "VS" = "#E30613", 
            "ZG" = "#006AB2", "ZH" = "#006AB2"
          )
          
          # Create a plot showing when each canton had an election
          p <- plot_ly() %>%
            layout(
              title = "Parlamentswahlen nach Kanton über die Zeit",
              xaxis = list(
                title = "Jahr",
                tickmode = "linear",
                dtick = 2
              ...
              yaxis = list(
                ...
                range = c(0, 1.5)  # Create some space for markers
              ),
              showlegend = TRUE,
              hovermode = "closest"
            )
          ...
          for (canton_nr in selected_cantons) {
            ...
            if (nrow(canton_data_filtered) > 0) {
              ...
              # Filter to only years with elections (where parl_election = 1)
              election_years <- canton_data_filtered %>% 
                filter(parl_election == 1) %>%
                pull(!!sym(year_col))
              
              if (length(election_years) > 0) {
                # Get color from canton_colors or use a default
                color <- if (!is.null(canton_colors[canton_abbrev])) {
                  canton_colors[canton_abbrev]
                } else {
                ...
                # Add scatter markers for each election year
                p <- p %>% add_trace(
                  x = election_years,
                  y = rep(1, length(election_years)),  # Fixed y value for visualization
                  name = canton_name,
                  type = "scatter",
                  mode = "markers",
                  marker = list(color = color, size = 10, symbol = "circle"),
                  hoverinfo = "text",
                  text = paste(canton_name, "<br>Parlamentswahl im Jahr:", election_years)
                )
              }
            }
          }
        ...
        # For all cantons view, count the number of elections per year
      ...
      if (input$variables == "reg_election") {
        # Check if we're showing specific cantons for government elections
        if (!is.null(selected_cantons) && input$election_type == "government") {
          # For government elections with specific cantons, show individual canton values
          canton_data <- data %>%
            # Convert "." to NA and then to numeric
            mutate(reg_election_cleaned = as.numeric(ifelse(reg_election == ".", NA, reg_election))) %>%
            filter(!is.na(reg_election_cleaned), kantonnr %in% selected_cantons) %>%
            # Add canton information
            mutate(
              canton_abbrev = sapply(kantonnr, get_canton_abbr),
              canton_name = sapply(canton_abbrev, function(abbr) get_canton_name(abbr, "de"))
          ...
          p <- plot_ly() %>%
            layout(
              title = "Regierungsratswahlen nach Kanton über die Zeit",
              xaxis = list(
                title = "Jahr",
                tickmode = "linear",
                dtick = 2
          ...
          for (canton_nr in selected_cantons) {
            ...
            if (nrow(canton_data_filtered) > 0) {
              ...
              # Filter to only years with elections (where reg_election_cleaned = 1)
              election_years <- canton_data_filtered %>% 
                filter(reg_election_cleaned == 1) %>%
                pull(!!sym(year_col))
              ...
              if (length(election_years) > 0) {
                ...
                p <- p %>% add_trace(
                  ...
                  marker = list(color = color, size = 10, symbol = "circle"),
                  hoverinfo = "text",
                  text = paste(canton_name, "<br>Regierungsratswahl im Jahr:", election_years)
                )
              }
            }
          }
